...
export interface DashboardResponse {
   ...
   *
   * 스펙의 "미결제 건수"는 매출(`TenantSale`)로는 여전히 셀 수 없다 — 그쪽은 **돈을 받은
   * 시점에만** 행이 생겨 외상 개념이 없다. 대신 job-063 부터 **이용권을 쓰지 못하고 받은
   * 등원**을 원장(ledger)의 `UNPAID_USE` 로 세어 내려준다. 잔액 0인 아이의 등원을 막지
   * 않기로 한 이상(현관에서 막으면 앱 밖에서 처리돼 기록 자체가 사라진다), 그렇게 지나간
   * 날이 어딘가에는 보여야 한다.
   */
  passes: {
    lowBalance: { petId: string; name: string; balance: number }[];
    /**
     * 이용권 없이 받은 등원 (job-063). **오늘 하루가 아니라 미정산 누계**다 —
     * 하루치만 보여주면 어제 넘어간 건은 영영 안 보인다.
     */
    unpaid: { petId: string; name: string; count: number }[];
  };
}
